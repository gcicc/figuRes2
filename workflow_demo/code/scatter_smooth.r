...
# Output Name: scatter_smooth.PDF
# Protocol ID: Protocol: Comp1234
# Plot Type: Scatterplot
# Figure Title: Scatterplot of Height vs Weight by Region with Smoothers
...
for.build <- ggplot(data=working.df, aes(x=HEIGHT, y=WEIGHT, color=REGRAP))+
  geom_point(alpha=.4)+
  stat_smooth(se=TRUE, size=.75, color="black",method = "lm")+
  guides(color=FALSE)+
