...
# Output Name: continuous_by_visit_and_treatment.PDF
# Protocol ID: Protocol: Comp1234
# Plot Type: Scatterplot
# Figure Title: Histogram with density by treatment and visit
...
filename <- "continuous_by_visit_and_treatment.PDF"
...
Week4 <- sample(size=nrow(working.df), working.df$VSBMI, replace=T)
Week8 <- sample(size=nrow(working.df), working.df$VSBMI, replace=T )
Week12 <- sample(size=nrow(working.df),working.df$VSBMI, replace=T)
Week16<- sample(size=nrow(working.df), working.df$VSBMI, replace=T )
Week20 <- sample(size=nrow(working.df),working.df$VSBMI, replace=T )
treatment <- sample(size=nrow(working.df),c("Placebo", "Treatment1", "Treatment2"), replace=T)

for.plot <- data.frame(Week4, Week8, Week12, Week16, Week20, treatment)
for.plot.melt <- melt(for.plot, measure.vars=1:5, id.vars=6)
head(for.plot.melt)
levels(for.plot.melt$variable) <- c("Week 4", "week 8", "Week 12", "Week 16", "Week 20")
levels(for.plot.melt$treatment) <- c("Placebo", "Treatment 1", "Treatment 2")
...
for.build <- ggplot(data=for.plot.melt, aes(x=value))+
