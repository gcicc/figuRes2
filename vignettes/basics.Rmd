...
What follows emphasizes figures2 functions that facilitate steps 4 and 5. As a reminder, this package was developed to accomadate large scale figure production with ggplot2/grid based graphics.

# The build.page and annotate.page functions
The build.page function is a wrapper for the the gridExtra::grid.arrange function.  It can be used to help visualize how a page will be partitioned. However, its primary purpose is to arrange a list of graphic objects on a page.

## Visualizing page partitioning
Recall the working definition for a figure is a collection of 1 or more graphics/tables assembled on a page with optional supporting metadata. Throughout this document we work under the assumption that figures are assembled on an 8.5 x 11 inch page with landscape orientation, though generalizing to other dimensions is straightforward. In the simplest case, a single graphic populates the page. The defaults of the build.page function allocate a predetermined value for left, right, top and bottom margins. These defaults provide sufficient space for the 3 lines of headers and the 5 lines for footnotes presumed by the the figuRes::annotate.page function. By adding a call to annotate.page, we can see how page division looks together with dummy headers and footers.
...
Suppose two graphics are to populate a figure.  E.g., forest plots typically juxtapose a graphic reporting on a stacked collection of confidence intervals with a table to to the right or left reporting summary statistics.  In designing a forest plot, one needs to decide

* how to allocate the page's real estate for the graphic, table, and margins 
* ensure proper alignment of labels, line segments and text,
* how to annotate the page with footnotes, headers, etc.
...
The figuRes::default.settings functions includes a call to the following ggplot2 function:
```{r}
theme_set(theme_grey2_nomargins())
```

The figuRes package includes a handful of themes which are slight variations on either the ggplot2 default theme, theme\_grey, or theme\_bw.  In particular, theme\_grey2\_default\_margins and theme\_grey2\_nomargins are two variations. Relative to theme_grey these themes have:
...
In some applications using the annotate.page function is inconventient. (See the example above where page margins were shrunken to be negligible.) In these cases, build the title into your ggplot object rather than use blank titles.

## Example 2: Assembling a figure with a 3x1 table of graphics

### Density curve with rug plot
Note that setting breaks to NULL removes the y-axis tick marks.

```{r}
ex.2a <- ggplot(data=working.df, aes(x=x, fill=group)) +
  geom_density(alpha=.4) +
  geom_rug(aes(color=group)) +
  labs(x="X",y=NULL, title="\nSubplot title 1", fill="Group", color="Group") +
  scale_y_continuous(breaks=NULL)
```

```{r, fig.cap="Example 2: Density and Rug Plot by Group"}
print(ex.2a)
```


## Example 3: Assembling a scatterplot with marginal densities
Intialize a session and generate data similar to the previous bar chart example.
...
* alters the size and tranparency of the symbols
* adds OLS lines with confidence bands
* alters labels
* changes symbols and colors
* manipulates teh limits and tick mark locations of the axes
...
This chunk creates densities for the y-values, suppressing labels and rotating the graphic.
...
#### Assembly of figure
The three graphics are now assembled and annotated to complete the figure. Note that object blankPanel is an object placed in the global environment by figuRes::default.settings.
